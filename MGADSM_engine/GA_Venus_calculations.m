function [v_out] = GA_Venus_calculations(Rperi, theta_b, v_inf_in, vE, et)
...
muV = 3.2486e5;      % Venus's gravitational parameter [km^3/s^2]

% Magnitude of incoming excess velocity
v_inf = norm(v_inf_in);

% Transform planet pole from body-fixed to J2000 at epoch
M_eq = cspice_pxform(['IAU_' 'VENUS'], 'J2000', et);   % body-fixed, equatorial -> J2000
...
B = sqrt(Rperi^2 + (2*muV*Rperi)/v_inf^2);

delta = 2 * atan2(muV, B * v_inf^2);    % Turning angle
