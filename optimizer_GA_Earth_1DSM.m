...
%   - Constraints   : circlcon_GA_dsm(x) (user-defined)
...
tf_guess = cspice_str2et(datestr(datetime(2032, 11, 17)));
...
minTOF_sec  = minTOF_days * 86400;        % seconds
maxTOF_sec  = maxTOF_days * 86400;        % seconds
...
nonlcon = @circlcon_GA_dsm;  % Enforces nonlinear constraints
J       = @cost_GA_1DSM;     % Returns scalar cost 
...
disp('theta_b [rad]');            disp(theta_b_opt)
...
%rng(42,'twister');    % (optional) make the run deterministic for reproducibility
for iter = 1:numIterations
    ...
    newGuess(5) = wrapPi(newGuess(5));   % keep theta_b in (-pi, pi]
...
disp('Best theta_b [rad] achieved'); disp(bestSolution(5));
