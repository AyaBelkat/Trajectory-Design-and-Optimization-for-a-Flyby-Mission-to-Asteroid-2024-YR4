function [c, ceq] = circlcon_GA_Venus_dsm(x)

% =========================================================================
% Nonlinear constraints for a Venus GA+DSM heliocentric transfer
%
...
%       eta1    : fraction of TOF at which GA occurs (Venus→GA duration = eta1*TOF)
%       eta2    : fraction of post-GA leg when DSM occurs (GA→DSM = eta2*(1-eta1)*TOF)
%       theta_b : B-plane angle selector for the GA (rad)
%       Rperi   : Venus flyby periapsis radius (km) (>= safety margin)
%
...
%     incoming v∞ at Venus pericenter (classical unpowered flyby model).
...
% -------------------- Ephemerides: Earth at departure and GA at Venus --------------
...
% This delivers v1 (at rE1) and v2 (at rV2); multiple-rev branches may exist.
...
v_out  = GA_Venus_calculations(Rperi, theta_b, v_inf2, vV2, tGA);   % heliocentric velocity after GA
...
% Convenience vectors for geometry checks (arrival on the Sun side of the
% asteroid)
vinf_dep = v_inf1_n;
% --- Arrival geometry  ------------------------------------------
...
%min_TOF = 100; % Minimum number of days for time of flight
c = [];
...
c(end + 1) = vinf_dep - vinfMax0;          % cap departure v_inf
